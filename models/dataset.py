...
class Dataset:
    def __init__(self, conf):
        ...
        self.depths_np = np.stack(
            [np.load(im_name)[..., None] for im_name in self.depth_lis]
        )
        ...
        self.depths = torch.from_numpy(
            self.depths_np.astype(np.float32)
        ).cpu()  # [n_images, H, W, 1]
        ...
        self.camera_center = [
            self.intrinsics_all[0][0, 2].cpu(),
            self.intrinsics_all[0][1, 2].cpu(),
        ]
        self.pose_all = torch.stack(self.pose_all).to(self.device)  # [n_images, 4, 4]
        self.pose_all_gt = torch.stack(self.pose_all_gt).to(
            self.device
        )  # [n_images, 4, 4]
        ...
        if self.init_pose:
            self.pose_network = LearnPose(
                num_cams=len(self.pose_all),
                init_c2w=self.pose_all,
                trans_offset=self.trans_offset,
            ).to(self.device)
        else:
            self.pose_network = LearnPose(
                num_cams=len(self.pose_all),
        ...
        if self.init_intrinsic:
            self.intrinsic_network = LearnFocal(
                H=self.H,
                W=self.W,
                req_grad=self.learn_intrinsic,
                fx_only=False,
                init_focal=self.focal,
                init_center=self.camera_center,
            ).to(self.device)
        else:
            self.intrinsic_network = LearnFocal(
                H=self.H,
        ...
        self.distortion_network = LearnDistortion(
            ...
            learn_shift=self.learn_shift,
            fix_scaleN=self.fix_scaleN,
        ...
        self.pc_scaled_depths = [
            F.interpolate(
                torch.unsqueeze(torch.unsqueeze(depth[:, :, 0], 0), 0),
                self.pc_resolution,
                mode="nearest",
            )
    ...
    def gen_rays_at(self, img_idx, resolution_level=1):
        ...
        p = torch.matmul(
            self.intrinsic_network(inverse=True)[:3, :3], p[:, :, :, None]
        ).squeeze()  # W, H, 3
        rays_v = p / torch.linalg.norm(p, ord=2, dim=-1, keepdim=True)  # W, H, 3
        rays_v = torch.matmul(
            self.pose_network(img_idx)[:3, :3], rays_v[:, :, :, None]
        ).squeeze()  # W, H, 3
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        if self.type_src == "mvs":
            ...
            src_idxs = [self.pair[src_id]["index"] for src_id in src_ids][
                : self.num_src
            ]
        else:
            src_idxs = [img_idx.item() - 1, (img_idx.item() + 1) % self.n_images]
        depth_cam = torch.stack(
            [self.pose_network(img_idx), self.intrinsic_network()], dim=0
        )
        ...
        src_cams = torch.stack(
            [
                scale_camera(
                    torch.stack(
                        [self.pose_network(i), self.intrinsic_network()], dim=0
                    ),
                    self.feat_img_scale,
                )
                for i in src_idxs
        ...
        return (
            torch.cat(
                [rays_o.cpu(), rays_v.cpu(), color, mask[:, :1], depth.cpu()], dim=-1
            ).cuda(),
            feat_input,
            pc_input,
        )  # batch_size, 11
    ...
    def gen_rays_between(self, idx_0, idx_1, ratio, resolution_level=1):
        ...
        rays_v = p / torch.linalg.norm(p, ord=2, dim=-1, keepdim=True)  # W, H, 3
        trans = (
            self.pose_network(idx_0)[:3, 3] * (1.0 - ratio)
            + self.pose_network(idx_1)[:3, 3] * ratio
        )
