...
from utils.geometry_utils import extract_geometry, find_surface_points, sample_pdf
...
class NeuSRenderer:
    ...
    def render_core(
        ...
        if (
            depth_from_inside_only
            or object_mask_type == "rendering_foreground_network_mask"
        ):
            # [N_rays, N_inside]
            weights_in = (
                alpha_in
                * torch.cumprod(
                    torch.cat([torch.ones([batch_size, 1]), 1.0 - alpha_in + 1e-7], -1),
                    -1,
                )[:, :-1]
            )
        ...
        # Compute normals
        normal = (
            gradients.reshape(batch_size, n_samples, 3) * weights[:, :n_samples, None]
        )
        ...
        if pc_input is not None:
            ...
            ref_pc = transform_pixel_to_world(
                pc_pixels, ref_depth, ref_camera_mat, ref_world_mat
            )
            ...
            src_1_pc = transform_pixel_to_world(
                pc_pixels, src_1_depth, src_1_camera_mat, src_1_world_mat
            )
            ...
            src_2_pc = transform_pixel_to_world(
                pc_pixels, src_2_depth, src_2_camera_mat, src_2_world_mat
            )
        ...
        if feat_input is not None:
            ...
            feat_loss = get_feat_loss(
                ...
                feat_input["cam"].unsqueeze(0),
                feat_input["feat_src"].unsqueeze(0),
                feat_input["src_cams"].unsqueeze(0),
                feat_input["size"].unsqueeze(0),
                feat_input["center"].unsqueeze(0),
