    ...
    def get_depth_loss(self, depth_pred, depth_gt, mask):
        if self.use_ssi_depth_loss:
            depth_loss = self.depth_loss(
                depth_pred.reshape(1, 16, 32), depth_gt.reshape(1, 16, 32), mask.reshape(1, 16, 32)
            )
        else:
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        runner.validate_mesh(world_space=False, resolution=1024, threshold=args.mcube_threshold)
