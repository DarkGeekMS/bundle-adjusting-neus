...
def export_learned_cameras(conf):
    ...
    if init_pose:
        pose_network = LearnPose(
            num_cams=len(images_list), init_c2w=torch.stack([torch.zeros((4, 4)) for _ in range(len(images_list))], dim=0)
        ).to('cuda')
    else:
    ...
    model_list.sort()
    checkpoint = torch.load(os.path.join(base_exp_dir, 'checkpoints', model_list[-1]), map_location='cuda')
    ...
    for cam_id in range(len(images_list)):
        camera_parameters[f'pose_mat_{cam_id}'] = pose_network(cam_id).detach().cpu().numpy()
        camera_parameters[f'intrinsic_mat_{cam_id}'] = intrinsic_network().detach().cpu().numpy()
